<template>
	<view class="content accountSet">
		<view class="accountSet_main_list">
			<view class="c_block" @click="$goPage('../modify/modify?type=1')">
				<view class="left">
					<image src="../../../static/s1.png" mode="aspectFit"></image>
					<text>登录密码</text>
				</view>
				<view class="right">
					<view class="btn_active" @click="showResetPwd = true">修改</view>
					<uni-icons type="arrowright" :color="'#C5CFD5'" size="24"></uni-icons>
				</view>
			</view>
			<view class="c_block" @click="$goPage('../modify/modify?type=2')">
				<view class="left">
					<image src="../../../static/s2.png" mode="aspectFit"></image>
					<text>资金密码</text>
				</view>
				<view class="right">
					<view class="btn_default" v-if="USERINFO.setPayPassword">重置</view>
					<view class="btn_active" v-if="USERINFO.setPayPassword">修改</view>
					<view class="btn_active" v-if="USERINFO.setPayPassword == false">设置</view>
					<uni-icons type="arrowright" :color="'#C5CFD5'" size="24"></uni-icons>
				</view>
			</view>
			<view class="c_block" @click="$goPage('../modify/modify?type=3')">
				<view class="left">
					<image src="../../../static/s3.png" mode="aspectFit"></image>
					<text>邮箱验证</text>
				</view>
				<view class="right">
					<view class="btn_default" style="color: #D14B64;">未绑定</view>
					<uni-icons type="arrowright" :color="'#C5CFD5'" size="24"></uni-icons>
				</view>
			</view>
			<view class="c_block"  v-if="REG_EMAIL.test(USERINFO.account)">
				<view class="left">
					<image src="../../../static/s3.png" mode="aspectFit"></image>
					<text>手机号</text>
				</view>
				<view class="right">
					...
					<view v-else class="btn_active" @click="isSetPhone = true">设置</view>
				</view>
			</view>
			<view class="c_block">
				<view class="left">
				...
				<view class="right">
					<switch @change="setAvoid" :checked="AVOID == 1 ? true : false" style="transform:scale(0.6);" color="#03bcc0" />
				</view>
			</view>
			<view class="c_block"  v-if="REG_EMAIL.test(USERINFO.account)">
				<view class="left">
			...
			<view class="c_block">
				...
				<view class="right">
					<view class="btn_default" v-if="GOOGLE.isBound && GOOGLE.status == 'ON'" @click="isPCReset = true" style="color: #000;">重置</view>
					<switch @change="$goPage('../google/google')" :checked="GOOGLE.status == 'ON' ? true : false" style="transform:scale(0.6);" color="#03bcc0" />
				</view>
			</view>
		</view>
<!-- 
      ...
      <view class="accountSet_main_list_item clear">
		...
		<!-- 设置/修改资金密码 -->
		<Modal v-if="showResetPayPwd" :title="USERINFO.setPayPassword ? '修改资金密码' : '设置资金密码'" @close="closeModal('showResetPayPwd')">
		...
		<!-- 输入资金密码 -->
		<Modal :title="'开启免密支付'" v-if="isPayPwd" @close="closeModal('isPayPwd')">
			<view class="common_model">
				<view class="model_input">
					<input type="password" placeholder-class="model_pla" v-model="payPwd" placeholder="请输入资金密码" />
				</view>
...
export default {
	components: { uniIcons, uniPopup, HMmessages, Modal },
	mixins: [main],
	computed: {
