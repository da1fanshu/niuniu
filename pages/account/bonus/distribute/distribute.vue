  ...
  <view class="content">
    <view class="distribute">
      <view class="coin_number clear">
        <view class="fl">
          <image src="/static/ping.png" style="height: 34rpx; width: 34rpx; margin-right: 18rpx;" mode=""></image>
          <text>总币数量</text>
        </view>
        <view class="fr">
      ...
      <view class="coin_money clear">
        <text class="fl">红包类型为拼手气红包</text>
        <text class="fr">可用：{{ filter.fix(money, 4) }}</text>
      </view>
      <view class="coin_number clear">
    ...
    <!-- 密码输入 -->
    <Modal :title="'发红包'" :isMask="true" v-if="showPush" @close="closeModal('showPush')">
      <view class="common_model">
        <view class="common_form">
          ...
          <view class="item clear">
            <view class="fr" style="width: 100%;"><input type="password" v-model="pwd" placeholder="请输入资金密码" /></view>
          </view>
        </view>
        <view class="common_btn"><button class="active" @click="submits(true)">确定</button></view>
      </view>
    </Modal>
        ...
        if (!this.poster.finalPath) {
          ...
          const d = await getSharePoster({
            ...
            qrCodeArray: ({ bgObj, type, bgScale }) => {
              return [
                {
                  text: `https://lkex.co/#/?redCode=${that.redId}&type=red`,//生产环境
