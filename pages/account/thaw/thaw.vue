		...
		<view class="l-title">
			<view class="m-title">
				...
				<picker @change="set1" :value="reason1" :range="reasonList1">
				  <view style="color: #05babe;;" class="uni-input">{{ reasonList1[reason1] }}</view>
				</picker>
			</view>
			<view class="m-title">
				<text class="sm-title">状态</text>
				<picker @change="set2" :value="reason2" :range="reasonList2">
				  <view style="color: #05babe;;" class="uni-input">{{ reasonList2[reason2] }}</view>
				</picker>
			</view>
			<view class="m-title">
				<text class="sm-title">币种</text>
				<picker @change="set1" :value="coinType" :range="coinList">
				  <view style="color: #05babe;;" class="uni-input">{{ coinList[coinType] }}</view>
				</picker>
			</view>
		</view>
		<view class="record">
				...
				<view class="common_list_main">
					...
					<view id="deposit">
						<view class="c_l_m_title">
							<text style="max-width: 225upx;">币种</text>
							<text>数量</text>
							<text style="max-width: 150upx;">时间</text>
						</view>
						<view class="c_l_m_list">
							<view class="item" v-for="(item, index) in aList" :key="index">
								<text style="max-width: 225upx;">{{item.assetCode}}</text>
						...
						<view class="common_list_page">
							<uni-pagination @change="setPageNo($event, 'aPageNo')" :total="aTotal * pageSize" :showIcon="true"></uni-pagination>
						</view>
					</view>
				</view>
			</view>
